<!DOCTYPE html><!--  This site was created in Webflow. http://www.webflow.com  -->
<!--  Last Published: Mon Nov 02 2020 09:08:50 GMT+0000 (Coordinated Universal Time)  -->
<html data-wf-page="5ea837e8c81001b668dffd4a" data-wf-site="5ea837e8c8100167b2dffd49">

<head>
    <meta charset="utf-8">
    <title>{{ config('app.name') }}</title>
    <meta content="width=device-width, initial-scale=1" name="viewport">
    <meta content="Webflow" name="generator">
    <link href="{{ asset('css/cli/normalize.css') }}" rel="stylesheet" type="text/css">
    <link href="{{ asset('css/cli/webflow.css') }}" rel="stylesheet" type="text/css">
    <link href="{{ asset('css/cli/split-opl.webflow.css?ver=1.1') }}" rel="stylesheet" type="text/css">
    <script src="https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js" type="text/javascript"></script>
    <script type="text/javascript">
        WebFont.load({
            google: {
                families: ["Inter:regular,600", "Lora:regular"]
            }
        });
    </script>
    <!-- [if lt IE 9]><script src="https://cdnjs.cloudflare.com/ajax/libs/html5shiv/3.7.3/html5shiv.min.js" type="text/javascript"></script><![endif] -->
    <script type="text/javascript">
        ! function(o, c) {
            var n = c.documentElement,
                t = " w-mod-";
            n.className += t + "js", ("ontouchstart" in o || o.DocumentTouch && c instanceof DocumentTouch) && (n
                .className += t + "touch")
        }(window, document);
    </script>
    <link href="img/devicon.png" rel="shortcut icon" type="image/x-icon">
    <link href="img/webclip.jpg" rel="apple-touch-icon">
    <style type="text/css">
        body {
            -webkit-font-smoothing: antialiased;
        }
    </style>
</head>

<body class="body">
    <div class="columns w-row">
        <div class="leftcontent w-col w-col-6 w-col-stack">
            <div data-w-id="b84f5156-c6e2-fb1d-6606-98a08030a472" style="opacity:0" class="image"></div>
        </div>
        <div class="rightcontent w-col w-col-6 w-col-stack">
            <div data-w-id="3fd5aeb3-22da-ed60-7286-0d11f16597d3" style="opacity:0" class="content">
                <div class="name">Everson Felix Leite</div>
                <h1 class="tagline"><strong class="bold-text">// DEV</strong></h1>
                <p class="bio">
                    <a href="{{ route('login') }}">>> CRUD de Notícias</a>
                </p>
                <div class="links w-row">
                    <div class="column w-col w-col-4">
                        <div class="text-block-2">Contato</div>
                        <ul role="list" class="list w-list-unstyled">
                            <li>
                                <a target="_blank" href="https://www.linkedin.com/in/everson-felix-leite">Linkedin</a>
                            </li>
                            <li>
                                <a href="mailto:eversonfelix_mb@hotmail.com">Email</a>
                            </li>
                            <li>
                                <a target="_blank" href="https://github.com/eversonfx">Github</a>
                            </li>
                        </ul>
                    </div>
                    <div class="column-2 w-col w-col-4">
                        <!-- <div class="text-block-2">social</div>
                        <ul role="list" class="list w-list-unstyled">
                            <!-- <li>
                                <a href="#">Twitter</a>
                            </li>
                            <li>
                                <a href="#">Instagram</a>
                            </li>
                            <li>
                                <a href="#">Dribbble</a>
                            </li> -->
                        </ul> -->
                    </div>
                    <div class="w-col w-col-4">
                        <!-- <div class="text-block-2">network</div>
                        <ul role="list" class="list w-list-unstyled">
                            <!-- <li>
                                <a href="#">Link One</a>
                            </li>
                            <li>
                                <a href="#">Link Two</a>
                            </li>
                            <li>
                                <a href="#">Link Three</a>
                            </li> -->
                        </ul> -->
                    </div>
                </div>
                <div class="credit">
                    <script>
                        document.write(new Date().getFullYear());
                    </script> Everson - <a href="https://onepagelove.com/split" target="_blank"
                        class="credit-links">Split Template</a> by <a href="https://onepagelove.com" target="_blank"
                        class="credit-links">One Page Love</a>
                </div>
            </div>
        </div>
    </div>
    <script src="{{ asset('js/cli/jquery-3.5.1.min.js') }}" type="text/javascript" crossorigin="anonymous">
    </script>
    <script src="{{ asset('js/cli/webflow.js') }}"></script>
    <!-- [if lte IE 9]><script src="https://cdnjs.cloudflare.com/ajax/libs/placeholders/3.0.2/placeholders.min.js"></script><![endif] -->
</body>

</html>